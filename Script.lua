...
-- Função Annoy All Players - ATAQUE EM TODOS
local function annoyAllPlayers()
    local Players = game:GetService("Players")
    ...
    for _, targetPlayer in ipairs(Players:GetPlayers()) do
        if targetPlayer ~= LocalPlayer and targetPlayer.Character then
            ...
            if hrp then
                ...
                if gunScript then
                    -- POSIÇÕES ALEATÓRIAS MUITO MAIS EXTREMAS
                    local randomX = math.random(-50000000, 50000000)
                    local randomZ = math.random(-50000000, 50000000)
                    local randomY = math.random(1000000, 5000000)
...
-- Toggle "Annoy All Players" - ATAQUE EM TODOS
TabPlayers:AddToggle({
    Name = "Annoy All Players",
    Description = "Ataca TODOS os jogadores do servidor automaticamente",
    Default = false,
    Callback = function(state)
        if state then
            garantirArma(true)
            local loopId = {}
            loopsAtivos["Annoy All Players"] = loopId
            task.spawn(function()
                while loopsAtivos["Annoy All Players"] == loopId do
                    annoyAllPlayers()
                    task.wait(0.1)  -- Loop um pouco mais lento pra não travar
                end
            end)
        else
            loopsAtivos["Annoy All Players"] = nil
            garantirArma(false)
        end
    end
})
