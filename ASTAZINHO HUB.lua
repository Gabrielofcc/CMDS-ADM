...
local function stuckPlayer(targetPlayer)
    ...
    local args = {
        [1] = hrp,
        [2] = hrp,
        [3] = Vector3.new(0, -50000, 0), -- Força para BAIXO (valor mais realista)
        [4] = hrp.Position, -- Usa a posição atual do alvo
...
-- Função principal de "Annoy Player [BETA]"
local function annoyPlayer(targetPlayer)
    if not targetPlayer or not targetPlayer.Character then return end
    local hrp = targetPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not hrp then return end

    -- pega a arma (Assault) e o script de disparo
    local gunScript = LocalPlayer.Backpack:FindFirstChild("Assault")
        and LocalPlayer.Backpack.Assault:FindFirstChild("GunScript_Local")
        or (LocalPlayer.Character:FindFirstChild("Assault")
        and LocalPlayer.Character.Assault:FindFirstChild("GunScript_Local"))
    ...
    -- posições aleatórias X e Z (grandes pra "confundir" servidor)
    local randomX = math.random(-20000000, 20000000)
    local randomZ = math.random(-20000000, 20000000)

    -- micro variação no Y pra simular movimento (funciona mesmo parado)
    local randomY = math.random(-1, 1) * 0.05
    ...
    local args = {
        ...
        [4] = hitPos, -- posição com leve variação
        [5] = gunScript:FindFirstChild("MuzzleEffect"),
        [6] = gunScript:FindFirstChild("HitEffect"),
        [7] = 0,
        [8] = 0,
...
TabPlayers:AddToggle({
    ...
    Description = "quando o alvo andar F pra ele ! [isto e infinito, ant supera]",
    Default = false,
    Callback = function(state)
        if state then
            garantirArma(true)
            ...
            task.spawn(function()
                while loopsAtivos["Annoy Player [BETA]"] == loopId do
                    ...
                    task.wait(0.1) -- intervalo entre execuções
                end
            end)
        else
...
TabPlayers:AddToggle({
    Name = "Loop TP Spawn",
    Description = "Fica teleportando para o spawn repetidamente",
    Callback = function(Value)
        loopTP = Value
        if loopTP then
            task.spawn(function()
                -- Função para encontrar o spawn
                local function getSpawnCFrame()
                    local spawnLocations = {
                        ...
                        workspace:FindFirstChild("DefaultSpawn")
                    }
                    ...
                    -- Fallback para spawn do Brookhaven
                    return CFrame.new(-26.09, 5, 6.11)
                end
                ...
                while loopTP do
                    ...
                    if myChar and myChar:FindFirstChild("HumanoidRootPart") then
                        ...
                        myChar.HumanoidRootPart.CFrame = spawnCFrame
                    end
                    task.wait(0.1) -- Aumentei o wait para 0.1s para não travar muito
                end
            end)
        end
    end
...
TabPlayers:AddButton({
    Name = "Fling Portas [Beta]",
    Description = "Portas vêm até você primeiro, depois atacam o alvo selecionado",
    Callback = function()
        local Players = game:GetService("Players")
        local Workspace = game:GetService("Workspace")
        local RunService = game:GetService("RunService")
        ...
        if not TargetPlayer then
            warn("Jogador não encontrado: " .. selectedPlayer)
            return
        end
        ...
        if not TargetHRP then
            warn("HumanoidRootPart do alvo não encontrado!")
            return
        end

        -- FASE 1: BlackHole segue VOCÊ (portas vêm até você)
        ...
        -- Encontra portas para controlar e salva posições originais
        ...
        local function SetupDoor(part)
            ...
            if not string.find(part.Name:lower(), "door") then return end

            -- Salva informações originais da porta
            ...
            align.Attachment1 = baseAttachment  -- Começa seguindo VOCÊ (BlackHole)
            align.MaxForce = 1e20
        ...
        -- Encontra e configura portas
        for _, obj in ipairs(Workspace:GetDescendants()) do
            SetupDoor(obj)
        end

        if #ControlledDoors == 0 then
            warn("Nenhuma porta móvel encontrada!")
            BlackHole:Destroy()
            return
        end

        print("Portas vindo até você: " .. #ControlledDoors)

        -- Espera as portas chegarem em você (2 segundos)
        task.wait(2)

        -- FASE 2: Agora as portas vão para o ALVO
        print("Portas indo para o alvo: " .. TargetPlayer.Name)
        
        -- Cria attachment no ALVO
        ...
        -- Direciona TODAS as portas para o ALVO
        for _, doorData in ipairs(ControlledDoors) do
            if doorData.Align then
                doorData.Align.Attachment1 = targetAttachment
            end
        end

        -- Aguarda o fling acontecer
        local start = tick()
        local flingDetected = false
        local flingThreshold = 50 -- Velocidade mínima para considerar como fling

        while tick() - start < 6 do
            if TargetHRP.Velocity.Magnitude >= flingThreshold then
                flingDetected = true
                print("✅ FLING DETECTADO! Velocidade: " .. TargetHRP.Velocity.Magnitude)
                break
            end
            RunService.Heartbeat:Wait()
        end

        -- FASE 3: Retornar portas para suas posições originais
        print("Retornando portas para posições originais...")
        
        -- Função para restaurar uma porta à sua posição original
        local function RestoreDoor(doorData)
            if doorData.Part and doorData.Part.Parent then
                -- Remove controles de movimento
                if doorData.Align then 
                    doorData.Align:Destroy() 
                end
                if doorData.Torque then 
                    doorData.Torque:Destroy() 
                end
                
                -- Restaura propriedades originais
                doorData.Part.CanCollide = doorData.OriginalCanCollide
                doorData.Part.Anchored = doorData.OriginalAnchored
                
                -- Teleporta para posição original suavemente
                local tweenInfo = TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
                local tween = game:GetService("TweenService"):Create(doorData.Part, tweenInfo, {
                    CFrame = doorData.OriginalCFrame
                })
                tween:Play()
                
                -- Remove marcador após um tempo
                local marker = doorData.Part:FindFirstChild("havens2025_Attached")
                if marker then
                ...
                -- Remove attachment após um tempo
                if doorData.Attachment then
                    task.delay(2, function()
                        if doorData.Attachment and doorData.Attachment.Parent then
                            doorData.Attachment:Destroy()
                        end
                    end)
                end
            end
        ...
        -- Restaura todas as portas
        for _, doorData in ipairs(ControlledDoors) do
            RestoreDoor(doorData)
        end

        -- Limpeza final após restaurar todas as portas
        task.wait(3) -- Espera as animações terminarem
        ...
        print("Fling finalizado. Sucesso: " .. tostring(flingDetected) .. ". Portas retornadas às posições originais.")
    end
})

TabPlayers:AddButton({
    Name = "Boat - fling (Custom Range)",
    Description = "ataque rápido e personalizável",
    Callback = function()
        -- CONFIGURAÇÕES
        local attackDelay = 0.05   -- tempo entre ataques (segundos)
        local attackCount = 10     -- quantas vezes ataca
        local minY = 3             -- altura mínima (evita cair no void)
        ...
        local Vehicles = workspace:FindFirstChild("Vehicles")
        local spawnPos = RootPart and RootPart.CFrame or CFrame.new(1754, 5, 58)
        ...
        repeat task.wait() RootPart.CFrame = Seat.CFrame until Humanoid.Sit    
        ...
        if not TargetRP then return end    

        -- salva posição inicial do barco
        local originalPos = PCar.PrimaryPart.CFrame

        -- executa múltiplos ataques
        for i = 1, attackCount do
            if not PCar or not PCar.Parent or not TargetRP then break end

            -- garante que não vá muito baixo
            ...
            -- retorna pro ponto inicial
            task.wait(0.05)
            if PCar and PCar.Parent then
                PCar:SetPrimaryPartCFrame(originalPos)
            end
...
TabPlayers:AddButton({
    ...
    Callback = function()
        ...
        -- Função do V2: Segurar e girar em volta do alvo
        local function orbitTarget()
            local vel = TargetRP.Velocity.Magnitude
            local dir = TargetH.MoveDirection
            
            -- Segurar sempre 7 studs à frente do alvo
            local front = TargetRP.CFrame.LookVector * 7
            local basePosition = TargetRP.Position + front + Vector3.new(0, 1.5, 0)
            
            -- Aplicar rotações aleatórias para o efeito V2
            ...
            if PCar and PCar.PrimaryPart then
                PCar:SetPrimaryPartCFrame(CFrame.new(basePosition) * randomRotation)
            end
        end
        ...
        task.spawn(function()
            ...
            while true do
                task.wait(0.2)
                if PCar and PCar.Parent then
                    orbitTarget()  -- Usa a função orbit do V2 em vez do fling normal
                else
                    break
                end
            end
...
TabPlayers:AddToggle({
    ...
    Callback = function(state)
        ...
        if autoFling then
            task.spawn(function()
                while autoFling do
                    ...
                    if targetPlayer and targetPlayer.Character then
                        ...
                        -- Spawnar MilitaryBoatFree (igual ao que funciona)
                        game:GetService("ReplicatedStorage").RE:FindFirstChild("1Ca1r"):FireServer("PickingBoat", "MilitaryBoatFree")
                        task.wait(1)
                        
                        -- Esperar barco spawnar (CORREÇÃO)
                        local boat = nil
                        local attempts = 0
                        repeat
                            boat = workspace.Vehicles:FindFirstChild(Player.Name.."Car")
                            task.wait(0.1)
                            attempts = attempts + 1
                        until boat or attempts > 20
                        
                        if not boat then
                            warn("Falha ao spawnar barco")
                            return
                        end

                        -- Configurar barco para ataque (CORREÇÃO)
                        if not boat.PrimaryPart then
                            boat.PrimaryPart = boat:FindFirstChild("Body") and boat.Body:FindFirstChild("VehicleSeat") or boat:FindFirstChild("Body")
                        end

                        -- BodyVelocity (CORREÇÃO - aplicar no PrimaryPart)
                        if boat.PrimaryPart:FindFirstChildWhichIsA("BodyVelocity") then
                            boat.PrimaryPart:FindFirstChildWhichIsA("BodyVelocity"):Destroy()
                        end
                        ...
                        if troot and thum then
                            repeat
                                local pos = troot.Position + (troot.Velocity / 1.5)
                                boat:SetPrimaryPartCFrame(CFrame.new(pos))
                                task.wait()
                            until not autoFling or thum.Health <= 0 or not tchar:IsDescendantOf(Workspace) or targetPlayer.Parent ~= Players
                        end
                        
                        -- Deletar barco quando terminar
                        if boat then
                            local args = {[1] = "DeleteAllVehicles"}
                            game:GetService("ReplicatedStorage").RE:FindFirstChild("1Ca1r"):FireServer(unpack(args))
                        end
            ...
            -- Deletar barco quando desligar o toggle
            local args = {[1] = "DeleteAllVehicles"}
            game:GetService("ReplicatedStorage").RE:FindFirstChild("1Ca1r"):FireServer(unpack(args))
        end
    end
...
local TabFuncoesCriador = Window:MakeTab({ "Funções Criador", "👑" })

TabFuncoesCriador:AddSection({Name = "Controle de Tamanho"})

local tamanhoAtivo = false
local loopTamanho = nil
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

TabFuncoesCriador:AddToggle({
    Name = "Loop Alterar Tamanho (0.50-1.0)",
    Description = "Altera tamanho de todos menos você em loop infinito",
    Default = false,
    Callback = function(state)
        tamanhoAtivo = state
        
        if tamanhoAtivo then
            -- Iniciar o loop de alteração de tamanho
            loopTamanho = task.spawn(function()
                local tamanhos = {1.0, 0.95, 0.90, 0.85, 0.80, 0.75, 0.70, 0.65, 0.60, 0.55, 0.50}
                local index = 1
                local direcao = 1 -- 1 para aumentar, -1 para diminuir
                
                while tamanhoAtivo do
                    -- Aplicar para todos os jogadores exceto você
                    for _, player in ipairs(Players:GetPlayers()) do
                        if player ~= LocalPlayer and player.Character then
                            local humanoid = player.Character:FindFirstChildOfClass("Humanoid")
                            if humanoid then
                                -- Aplica o tamanho atual
                                humanoid.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None
                                humanoid.BodyDepthScale.Value = tamanhos[index]
                                humanoid.BodyHeightScale.Value = tamanhos[index] 
                                humanoid.BodyWidthScale.Value = tamanhos[index]
                                humanoid.HeadScale.Value = tamanhos[index]
                            end
                        end
                    end
                    
                    -- Atualizar índice para próximo tamanho
                    if direcao == 1 then
                        index = index + 1
                        if index > #tamanhos then
                            index = #tamanhos - 1
                            direcao = -1
                        end
                    else
                    ...
                    task.wait(0.3) -- Espera entre mudanças
                end
            end)
        else
            -- Parar o loop e resetar tamanhos
            if loopTamanho then
                task.cancel(loopTamanho)
                loopTamanho = nil
            end
            
            -- Resetar tamanhos de todos os jogadores
            for _, player in ipairs(Players:GetPlayers()) do
                if player.Character then
                    local humanoid = player.Character:FindFirstChildOfClass("Humanoid")
                    if humanoid then
                        humanoid.BodyDepthScale.Value = 1.0
                        humanoid.BodyHeightScale.Value = 1.0
                        humanoid.BodyWidthScale.Value = 1.0
                        humanoid.HeadScale.Value = 1.0
                    end
                end
            end
        end
...
TabFuncoesCriador:AddButton({
    Name = "Resetar Todos os Tamanhos",
    Description = "Volta todos ao tamanho normal",
    Callback = function()
        for _, player in ipairs(Players:GetPlayers()) do
            if player.Character then
                local humanoid = player.Character:FindFirstChildOfClass("Humanoid")
...
local function FlingBall(target)
    ...
    Ball.CanCollide = false
    Ball.Massless = true
    Ball.CustomPhysicalProperties = PhysicalProperties.new(0.001, 0, 0)
    ...
    if target ~= player then
        ...
        if tchar and tchar:FindFirstChild("HumanoidRootPart") and tchar:FindFirstChild("Humanoid") then
            ...
            bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)  -- ← FORÇA MÁXIMA
            bv.P = math.huge
            bv.Parent = Ball
            ...
            repeat
                if troot.Velocity.Magnitude > 0 then
                    local pos = troot.Position + (troot.Velocity / 0.0000000001)
                    Ball.CFrame = CFrame.new(pos)
                    Ball.Orientation += Vector3.new(360, 360, 360)
                else
                    for i, v in pairs(tchar:GetChildren()) do
                        if v:IsA("BasePart") and v.CanCollide and not v.Anchored then
                            Ball.CFrame = v.CFrame
                            task.wait(1/100000000000)
                        end
                    end
                end
                task.wait(1/100000000000)
            until troot.Velocity.Magnitude > 9999999999 or thum.Health <= 0 or not tchar:IsDescendantOf(workspace) or target.Parent ~= players
...
Tab3:AddButton({
    ...
    Callback = function()
        ...
        -- CORREÇÃO: BlackHole fica 6 studs EMBAIXO do player
        RunService.Heartbeat:Connect(function()
            if BlackHole and BlackHole.Parent then
                BlackHole.CFrame = HRP.CFrame * CFrame.new(0, -6, 0)
            end
        end)
        ...
        local function SetupPart(part)
            ...
            local align = Instance.new("AlignPosition", part)
            align.Attachment0 = a1
            align.Attachment1 = baseAttachment  -- Vai para 6 studs embaixo
            align.MaxForce = 1e20
            align.MaxVelocity = math.huge
            align.Responsiveness = 99999
        ...
        -- CORREÇÃO: Sistema para detectar quando portas chegam e esconder
        local function MonitorDoorsArrival()
            while #ControlledDoors > 0 do
                local doorsHidden = 0
                
                for _, data in ipairs(ControlledDoors) do
                    if data.Part and data.Part.Parent then
                        -- Calcula distância até o BlackHole (6 studs embaixo)
                        local distance = (data.Part.Position - (HRP.Position - Vector3.new(0, 15, 0))).Magnitude
                        
                        -- Se porta chegou perto (dentro de 3 studs), esconde
                        if distance < 15 then
                            data.Part.Transparency = 1  -- Torna invisível
                            doorsHidden = doorsHidden + 1
                        else
                            data.Part.Transparency = 0  -- Mantém visível se longe
                        end
                    end
                end
                
                -- Feedback
                if doorsHidden > 0 then
                    print("🚪 " .. doorsHidden .. " porta(s) escondida(s) embaixo de você")
                end
        ...
        -- Inicia monitoramento de chegada das portas
        task.spawn(MonitorDoorsArrival)
        ...
        local function FlingPlayer(player)
            ...
            -- Torna portas visíveis durante o ataque
            for _, data in ipairs(ControlledDoors) do
                if data.Part and data.Part.Parent then
            ...
            -- Retorna as portas para 6 studs embaixo e esconde
            for _, data in ipairs(ControlledDoors) do
                if data.Align then
                    data.Align.Attachment1 = baseAttachment
                end
                if data.Part and data.Part.Parent then
                    task.delay(1, function()  -- Espera um pouco antes de esconder
                        data.Part.Transparency = 1
                    end)
                end
            end
            
            print("🎯 Fling executado! Sucesso: " .. tostring(flingDetected))
        end
        ...
        -- Limpeza quando script terminar
        task.delay(300, function() -- 5 minutos
            if BlackHole and BlackHole.Parent then
                BlackHole:Destroy()
            end
            for _, data in ipairs(ControlledDoors) do
                if data.Part and data.Part.Parent then
                    data.Part.Transparency = 0  -- Restaura visibilidade
                end
            end
        end)
    end
